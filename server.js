...
const { calculateCoins } = require('./lib/scoring');

// Build the Express app around an injected pg pool. Taking the pool as a
// parameter (rather than constructing it at module scope) lets tests drive the
// routes with a fake pool — no real database or module mocking required.
function createApp(pool) {
  ...
  app.post('/api/progress/update', async (req, res) => {
    ...
    try {
      const coinsEarned = calculateCoins(correct, difficulty);
...
// Build the production app against a real Neon Postgres pool. Skipped under
// Vitest so importing this module for tests never opens a real connection;
// tests call createApp() with their own fake pool instead.
let app;
...
// The Vercel/api entry point requires this module and uses the export as the
// serverless handler, so the app must remain the primary export. createApp is
// attached for tests.
module.exports = app || {};
